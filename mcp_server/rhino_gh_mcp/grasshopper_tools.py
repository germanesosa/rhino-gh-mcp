...
class GrasshopperConnection:
    def __init__(self, host='localhost', port=9999):  # Using port 9999 to match gh_socket_server.py
        ...
        self.timeout = 60.0  # 30 second timeout
...
class GrasshopperTools:
        ...
        try:
            ...
            if isinstance(instance_guid, str) and (
                ...
                if payload and "instance_guid" in payload:
                    ...
                    instance_guid = str(payload.get("instance_guid") or "")
                    code = str(payload.get("code", code) or "")
                    description = str(payload.get("description", description) or "")
                    message_to_user = str(payload.get("message_to_user", message_to_user) or "")
    ...
    def update_script_with_code_reference(self, ctx: Context, instance_guid: str = "", file_path: str = "", 
                                        param_definitions: List[Dict[str, Any]] = [], description: str = "", 
                                        name: str = "", force_code_reference: bool = False) -> str:
    ...
    def recompute_all(self, ctx: Context, instance_guid: str) -> str:
        ...
        try:
            connection = get_grasshopper_connection()
            result = connection.send_command("recompute_all", {})
            if result.get("status") == "error":
                return json.dumps({"status": "error", "result": result.get("result", "Unknown error")})
            # Only return status/result, not full component info
            return json.dumps({"status": "success", "result": f"Grasshopper recomputed."})
        except Exception as e:
            return json.dumps({"status": "error", "result": f"Error recomputing component: {str(e)}"})
    ...
    def add_slider_to_canvas(self, ctx: Context, name: str, min_value: float, max_value: float, value: float, position_x: int = 100, position_y: int = 100, integer: bool = False) -> str:
        ...
        connection = get_grasshopper_connection()
        result = connection.send_command("add_slider_to_canvas", {
            "name": name,
            "min_value": min_value,
            "max_value": max_value,
            "value": value,
    ...
    def add_component_to_canvas(self, ctx: Context, component_name: str, position_x: int = 100, position_y: int = 100) -> str:
        ...
        connection = get_grasshopper_connection()
        result = connection.send_command("add_component_to_canvas", {
    ...
    def remove_node(self, ctx: Context, instance_guid: str) -> str:
        ...
        connection = get_grasshopper_connection()
        result = connection.send_command("remove_node", {
            "instance_guid": instance_guid
        })
        return json.dumps(result, indent=2)
    ...
    def get_all_component_library(self, ctx: Context) -> str:
        """Get all Grasshopper components as they appear in the UI ribbon layout."""
        connection = get_grasshopper_connection()
        result = connection.send_command("get_all_component_library")
        return json.dumps(result, indent=2)
    ...
    def set_component_parameter(self, ctx: Context, instance_guid: str = "", param_name: str = "", value: str = "") -> str:
        ...
        Args:
            instance_guid: The GUID of the component or parameter
            param_name: The name or nickname of the parameter
            value: The value to set (as string, will be converted as needed)
        Returns:
            JSON string with status/result
        """
        try:
            logger.info(f"Setting parameter: guid={instance_guid}, param={param_name}, value={value}")
            connection = get_grasshopper_connection()
            # Get parameter info (optional, for validation or future use)
            obj_info = connection.send_command("get_objects", {
                "instance_guids": [instance_guid],
                "context_depth": 0
            })
            if obj_info.get("status") != "success":
                return json.dumps({"status": "error", "result": f"Failed to get object info: {obj_info.get('result', '')}"})
            # Find the parameter in the result
            param_guid = None
            for guid, info in obj_info.get("result", {}).items():
                for param_list in (info.get("Inputs", []), info.get("Outputs", [])):
                    for param in param_list:
                        if param.get("name") == param_name or param.get("nickName") == param_name:
                            param_guid = param.get("instanceGuid")
                            break
            if not param_guid:
                return json.dumps({"status": "error", "result": f"Parameter '{param_name}' not found."})
            # Always send the set_component_parameter command to the backend, which will handle sliders, color swatches, boolean toggles, etc.
            set_result = connection.send_command("set_component_parameter", {
                "instance_guid": instance_guid,
                "param_name": param_name,
                "value": value
            })
            return json.dumps(set_result, indent=2)
        except Exception as e:
            return json.dumps({"status": "error", "result": f"Exception: {str(e)}"})
    ...
    def get_panel_content(self, ctx: Context, instance_guid: str) -> str:
        ...
        Args:
            instance_guid: The GUID of the Panel component to read
            
        Returns:
            JSON string with panel content and properties, including:
            - content: The text content of the panel
            - instance_guid: The panel's GUID
            - nickname: The panel's nickname
            - properties: Panel display properties including:
                - wrap: Whether items are wrapped or trimmed
                - multiline: Whether new lines create new data items
                - special_codes: Whether special codes are replaced
                - draw_paths: Whether branch paths are displayed
                - draw_indices: Whether item indices are displayed
                - stream_contents: Whether panel streams to a file
                - stream_path: File path for content streaming
                - alignment: Text alignment (Default, Left, Right)
                - colour: Background color as ARGB integer
                - colour_hex: Background color as hex string
        """
        try:
            if not instance_guid:
                return json.dumps({"status": "error", "result": "No instance_guid provided. Please specify the GUID of the Panel component."})
            
            logger.info(f"Getting panel content for GUID: {instance_guid}")
            connection = get_grasshopper_connection()
            result = connection.send_command("get_panel_content", {
                "instance_guid": instance_guid
            })
